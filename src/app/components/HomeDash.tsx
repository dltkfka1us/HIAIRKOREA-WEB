          ...
          <div className="home-card-content loaded p-4 flex-1 flex flex-col justify-between" id="home-edu-content">
            <div className="kpi-mini-row grid grid-cols-4 gap-2 mb-3">
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                ...
                <span className="value block text-xl font-black text-slate-800">{eduData?.total?.target || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">이수</span>
                <span className="value block text-xl font-black text-emerald-500">{eduData?.total?.done || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">미이수</span>
                <span className="value block text-xl font-black text-rose-500">{eduData?.total?.undone || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">이수율</span>
                <span className="value block text-xl font-black text-blue-500">{eduData?.total?.rate || 0}<small className="text-xs font-semibold ml-0.5">%</small></span>
              </div>
            </div>
        ...
        <div className="content-card kpi-dashboard-card border border-slate-200/80 shadow-sm rounded-[20px] bg-white overflow-hidden h-[340px] flex flex-col transition hover:shadow-md">
          ...
          <div className="home-card-content loaded p-4 flex-1 flex flex-col justify-between" id="home-tbm-content">
            <div className="kpi-mini-row grid grid-cols-4 gap-2 mb-3">
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">대상</span>
                <span className="value block text-xl font-black text-slate-800">{tbmData?.total?.target || 0}<small className="text-xs font-semibold ml-0.5">팀</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">완료</span>
                <span className="value block text-xl font-black text-emerald-500">{tbmData?.total?.done || 0}<small className="text-xs font-semibold ml-0.5">팀</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">미실시</span>
                <span className="value block text-xl font-black text-rose-500">{tbmData?.total?.undone || 0}<small className="text-xs font-semibold ml-0.5">팀</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">실시율</span>
                <span className="value block text-xl font-black text-purple-500">{tbmData?.total?.rate || 0}<small className="text-xs font-semibold ml-0.5">%</small></span>
              </div>
            </div>
        ...
        <div className="content-card kpi-dashboard-card border border-slate-200/80 shadow-sm rounded-[20px] bg-white overflow-hidden flex flex-col h-[340px] transition hover:shadow-md">
          ...
          <div className="home-card-content loaded p-4 flex flex-col flex-1 overflow-hidden" id="home-legal-content">
            <div className="kpi-mini-row grid grid-cols-4 gap-2 mb-2 shrink-0">
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">대상</span>
                <span className="value block text-xl font-black text-slate-800">{legalData?.total?.target || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">이수</span>
                <span className="value block text-xl font-black text-emerald-500">{legalData?.total?.done || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">미이수</span>
                <span className="value block text-xl font-black text-rose-500">{legalData?.total?.undone || 0}<small className="text-xs font-semibold ml-0.5">명</small></span>
              </div>
              <div className="kpi-mini-item text-center bg-slate-50/80 rounded-xl py-2">
                <span className="label block text-[11px] font-bold text-slate-500 mb-1">이수율</span>
                <span className="value block text-xl font-black text-blue-500">{legalData?.total?.rate || 0}<small className="text-xs font-semibold ml-0.5">%</small></span>
              </div>
            </div>

            {/* Person Icons Visualiser */}
            <div className="flex items-center justify-center gap-2 my-1 py-1 shrink-0 bg-slate-50/70 rounded-xl border border-gray-100">
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 bg-slate-50/60 p-2 rounded-xl border border-gray-100 max-h-[125px]">
              {legalData?.officers?.map((officer: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between text-xs p-2 bg-white rounded-lg border border-gray-100 shadow-sm">
